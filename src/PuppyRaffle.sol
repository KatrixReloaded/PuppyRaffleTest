...
contract PuppyRaffle is ERC721, Ownable {
    ...
    //@audit Ownable not initialized, contract has no owner
    ...
    function enterRaffle(address[] memory newPlayers) public payable {
        //q what if array length is zero?
    ...
    //@audit 0 index posn. has an active player address stored, should not return 0 for inactive players
    ...
    function selectWinner() external {
        ...
        //@audit randomness (Weak PRNG - Pseudo-Random Number Generator)
        //q what if we don't like the winner? do we revert?
        //q gas war... //@followup
        uint256 winnerIndex =
            uint256(keccak256(abi.encodePacked(msg.sender, block.timestamp, block.difficulty))) % players.length;
        address winner = players[winnerIndex];
        uint256 totalAmountCollected = players.length * entranceFee;
        //q is it supposed to be 80%?
        //q might be an arithmetic error here
        ...
        //@audit blank spots in the array (due to refund() function) means the possibility of prizePool being transferred to address(0)
        //fix: add a check to see if players[winnerIndex] == address(0) and revert if true
